...
注：实际匹配时会去除方程中的空格，以下仅为了方便阅读。
- ...ij, ...jc -> ...ic (矩阵乘法)
- ...ic, ...jc -> ...ij (矩阵乘法，需要转置第二个输入)
- ...hic,...hjc -> ...hij (批量矩阵乘法，需要转置第二个输入)
- ...nij, ijc -> ...nic (批量矩阵乘法，复杂)
- aijk, aijh -> ajkh (批量矩阵乘法，需要转置输入并合并 a,j 维度)
"""
...
def get_tensor_shape_raw(model, tensor_name):
    """
    获取张量的形状
...
def get_tensor_shape(model, tensor_name, default_shape=1):
    shape = get_tensor_shape_raw(model, tensor_name)
    if shape is None:
        raise ValueError(f"Cannot find shape for tensor {tensor_name}")
    
    for i, d in enumerate(shape):
        if d is None or isinstance(d, str) or d == 0:
            print(f'  警告: 张量 {tensor_name} 的维度 {i} 不可用 {d}，使用默认值 {default_shape}')
            shape[i] = default_shape
            ...
            if node.op_type == 'Einsum':
                ...
                for attr in node.attribute:
                    if attr.name == 'equation':
                        equation = attr.s.decode('utf-8').replace(' ', '')  # 去除空格
                        break
                ...
                if '...ij,...jc->...ic' in equation:
                ...
                elif 'aijk,aijh->ajkh' in equation:
                    # 批量矩阵乘法，需转置并合并 aj 通道
                    ...
                    j = input1_shape[2]
                    k = input1_shape[3]
                    h = input2_shape[3]   # input2: [a, i, j, h]
                    B = a * j

                    # 步骤1: 转置输入1 [a, i, j, k] -> [a, j, k, i]
                    input1_transposed_name = f'{input1_name}_transposed_{node.name}'
                    transpose1_node = helper.make_node(
                        'Transpose',
                        inputs=[input1_name],
                        outputs=[input1_transposed_name],
                        name=f'{node.name}_transpose1',
                        perm=[0, 2, 3, 1]  # [a, i, j, k] -> [a, j, k, i]
                    )
                    new_nodes.append(transpose1_node)

                    # 步骤2: 转置输入2 [a, i, j, h] -> [a, j, i, h]
                    input2_transposed_name = f'{input2_name}_transposed_{node.name}'
                    transpose2_node = helper.make_node(
                        'Transpose',
                        inputs=[input2_name],
                        outputs=[input2_transposed_name],
                        name=f'{node.name}_transpose2',
                        perm=[0, 2, 1, 3]  # [a, i, j, h] -> [a, j, i, h]
                    )
                    new_nodes.append(transpose2_node)

                    # 步骤3a: 合并批处理维度 输入1 [a, j, k, i] -> [B, k, i]
                    input1_reshaped_shape = [B, k, i]
                    input1_reshaped_name = f'{input1_name}_reshaped_{node.name}'
                    shape_tensor1 = numpy_helper.from_array(
                        np.array(input1_reshaped_shape, dtype=np.int64),
                        name=f'{input1_reshaped_name}_shape'
                    ...
                    # 步骤3b: 合并批处理维度 输入2 [a, j, i, h] -> [B, i, h]
                    input2_reshaped_shape = [B, i, h]
                    input2_reshaped_name = f'{input2_name}_reshaped_{node.name}'
                    shape_tensor2 = numpy_helper.from_array(
                        np.array(input2_reshaped_shape, dtype=np.int64),
                        name=f'{input2_reshaped_name}_shape'
                    ...
                    reshape2_node = helper.make_node(
                        'Reshape',
                        inputs=[input2_transposed_name, shape_tensor2.name],
                        outputs=[input2_reshaped_name],
                        name=f'{node.name}_reshape2'
                    )
                    new_nodes.append(reshape2_node)

                    # 步骤4: MatMul [B, k, i] x [B, i, h] -> [B, k, h]
                    matmul_name = f'{output_name}_matmul'
                    matmul_node = helper.make_node(
                        'MatMul',
                        inputs=[input1_reshaped_name, input2_reshaped_name],
                    ...
                    # 步骤5: 恢复形状 [B, k, h] -> [a, j, k, h]
                    output_reshaped_name = output_name  # 直接输出到最终节点名
                    output_shape_list = [a, j, k, h]
                    shape_tensor3 = numpy_helper.from_array(
                        np.array(output_shape_list, dtype=np.int64),
                        name=f'{output_reshaped_name}_shape'
                    )
                    new_initializers.append(shape_tensor3)
                    reshape3_node = helper.make_node(
                        'Reshape',
                        inputs=[matmul_name, shape_tensor3.name],
                        outputs=[output_reshaped_name],
                        name=f'{node.name}_reshape3'
                    )
                    new_nodes.append(reshape3_node)

                    replaced_count += 1
                    print(f'  已替换为 Transpose + Reshape + MatMul + Reshape (合并 a,j 维度)')
                    print(f'    输入1: {input1_shape} -> [a, j, k, i] -> {input1_reshaped_shape}')
                    print(f'    输入2: {input2_shape} -> [a, j, i, h] -> {input2_reshaped_shape}')
                    print(f'    MatMul: {input1_reshaped_shape} x {input2_reshaped_shape} -> [B, k, h]')
                    print(f'    输出: [B, k, h] -> {output_shape_list}')
